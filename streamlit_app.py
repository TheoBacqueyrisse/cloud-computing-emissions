...
if selected_option:
  ...
  if selected_option == 'Numpy':
    ...
    # Load the Data
    df = pd.read_csv(f'/content/drive/MyDrive/Data Mining Data/{str.lower(selected_option)}_data_with_Co2.csv')
  ...
  if selected_option == 'Pandas':
    ...
    st.markdown(f"## Chosen Repository : {selected_option} (only 50,000 last runs)", unsafe_allow_html=True)
    ...
    with col1:
      ...
      custom_labels = ['pull_request', 'push', 'issue_comment', 'schedule', 'workflow_dispatch']
      custom_colors = ['#011f4b', '#03396c', '#005b96', '#6497b1', '#b3cde0']
    ...
    with col2:
      ...
      custom_labels = ['skipped', 'success', 'cancelled', 'failure', 'action_required', 'startup_failure']
      custom_colors = ['#011f4b', '#03396c', '#005b96', '#6497b1', '#b3cde0', '#ffffff']
